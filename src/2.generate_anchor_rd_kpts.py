...
def main():
    ...
    for fname in frame_filenames:
        ...
        # Step 1: 在图像上随机采样 1024 个点
        H, W = image0.shape[:2]
        num_points = 1024
        xs = np.random.randint(0, W, size=num_points)
        ys = np.random.randint(0, H, size=num_points)
        sampled_kp = np.stack([xs, ys], axis=1)

        # Step 2: 提取特征
        feat_map = og.dino_extract.forward(image0)  # shape: (H_feat, W_feat, C)
        ...
        selected_features = get_patch_features(feat_map, sampled_kp, image0.shape)
        ...
        print(selected_features.shape, reduced.shape,sampled_kp.shape)
        # Step 4: 聚类
        joint_feat = np.concatenate([
            selected_features,
            np.array(sampled_kp) * 0.05  # 位置缩放以保持尺度匹配
        ], axis=1)
        ...
        with open(save_path, "w") as f:
            for (x, y), lbl in zip(sampled_kp, labels):
                label_str = f"cluster{lbl}" if lbl != -1 else "noise"
                f.write(f"{x:.2f},{y:.2f},{label_str}\n")
        img0_keypoints = visualize_keypoints_by_cluster(image0, sampled_kp, labels)
