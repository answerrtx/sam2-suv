...
class DINOExtract:
    ...
    def __init__(self, cpt_path: str, feature_layer: int = 1):
        ...
        state_dict_raw = torch.load(cpt_path, map_location=device)
...
def _preprocess_shape(
    ...
    if not isinstance(h_resize, torch.Tensor):
        h_resize = torch.tensor(h_resize)
    if not isinstance(w_resize, torch.Tensor):
        w_resize = torch.tensor(w_resize)
