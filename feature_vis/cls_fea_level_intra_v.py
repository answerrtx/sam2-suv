...
def extract_features(img_tensor, model):
    ...
    return outputs['x_norm_patchtokens'].squeeze(0).cpu()  # shape: [1146, 768]
...
def process_dataset(root_dir, suv_json, max_per_folder=50):
    ...
    for sub in tqdm(root_dir):
        ...
        for img_name in selected_imgs:
            ...
            feats = extract_features(image_tensor, model)  # shape: [1146, 768]
            ...
            for color, fine_label in color_map.items():
                ...
                patch_ids = map_coords_to_patch(coords, img_size=(224, 224), patch_grid=(14, 14))
...
def visualize(features1, labels1, features2, labels2, out_path="tsne_dual_dino_features.png"):
    ...
    tsne1 = TSNE(n_components=2, perplexity=30, random_state=42).fit_transform(pca1)

    pca2 = PCA(n_components=50).fit_transform(features2)
    tsne2 = TSNE(n_components=2, perplexity=30, random_state=42).fit_transform(pca2)
